...
// Lazy-load Spline only if needed
...
class SoftErrorBoundary extends React.Component {
  ...
  componentDidCatch() {
    // Silent failover to fallback UI
  }
  render() {
...
export default function Hero() {
  ...
  const hasSpline = useMemo(() => typeof sceneUrl === 'string' && sceneUrl.trim().length > 0 && sceneUrl.includes('.spline'), [sceneUrl])
        ...
        <div className="grid lg:grid-cols-12 gap-10 items-center">
          ...
          <div className="lg:col-span-6 relative">
            <div className="relative h-[420px] rounded-3xl overflow-hidden">
              {hasSpline && !prefersReducedMotion ? (
                <Suspense fallback={<div className="absolute inset-0 rounded-3xl bg-white/70 border border-white/60 backdrop-blur-xl shadow-[0_8px_30px_rgb(0,0,0,0.08)]" />}>
                  <SoftErrorBoundary fallback={<VisualFallback />}>
